...
function relativeDate(dateStr, lang = 'cs') {
  ...
  const labels = {
    cs: {
      today: 'dnes',
      day: (n) => n === 1 ? 'před 1 dnem' : `před ${n} dny`,
      week: (n) => n === 1 ? 'před 1 týdnem' : n < 5 ? `před ${n} týdny` : `před ${n} týdny`,
      month: (n) => n === 1 ? 'před 1 měsícem' : n < 5 ? `před ${n} měsíci` : `před ${n} měsíci`,
      year: (n) => n === 1 ? 'před 1 rokem' : n < 5 ? `před ${n} lety` : `před ${n} lety`,
    },
    en: {
      today: 'today',
    ...
    de: {
      today: 'heute',
      day: (n) => n === 1 ? 'vor 1 Tag' : `vor ${n} Tagen`,
  ...
  if (days < 7) return l.day(days);
  if (days < 30) return l.week(weeks);
  if (months < 12) return l.month(months);
  return l.year(years);
}
...
export default function ReferencesSection() {
  ...
  return (
    <section id="references" className="relative py-16 bg-secondary/20 overflow-hidden">
